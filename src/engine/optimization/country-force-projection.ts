...
function getBatchSize(unitId: string, catalog: UnitCatalog): number {
  return (
...
export function computeCountryForceProjection(input: CountryForceProjectionInput): CountryForceProjectionResult {
  ...
  const citySlots: CityForceProjectionSlot[] = foldResult.citySlots.map((slot: CityMobSlot) => {
    ...
    // The real flip point: the latest hour this city can still be running eco
    // builds before it must switch to military infra to hit the deadline.
    ...
    const { steps: infraSteps, mobOpenHour: infraDoneHour } = buildCityInfraSteps(
      primaryUnitId, slot.roLevel, flipPointAbsHour, planWeights, catalog, buildings,
    );
